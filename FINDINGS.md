...
进一步扫描 K∈{5,10,15,20,25,30,40,50}：K=5 平均恶化 0.0194°，K=10 的 5% 分位
仍为负，K≥15 后重采样 5% 分位转正。最佳点为 **K=40**，50 次扫描平均改善 0.0668°、
标准差 0.0032°。追加 100 次确认后，base 1.2176° → personalized **1.1504°**，平均改善
**0.0672°**，5%/95% 分位为 +0.0619°/+0.0725°，以 0.001° 为门槛的用户-重复
win rate 为 80.0%。K=50 反而回落到 0.0593°，说明保留部分样本给 calibration-validation
门控、避免把全部 pool 用满，比盲目增加拟合样本更稳。

以上仍基于同一个普适 checkpoint 的 6 个 held-out SID。已经启动 5 折 subject-disjoint
训练与个性化评测，使 56 个 SID 各作为测试用户一次；在该交叉验证完成前，不把上述收益
升级为全数据集结论。

### 更强普适基线对照：个性化收益依赖 checkpoint

对原 PIR 消融中精度更高的 `ablation_pir_no_constraint/best.pt` 使用完全相同的缓存、
split 和重采样扫描。无约束普适模型在 interleaved evaluation 上的 macro mean 已由
受约束 checkpoint 的 1.2076° 降到 **0.9180°**。此时最佳个性化结果只有 interleaved
bias K=50 的 **0.0188°** 改善；chronological 的所有适配器均为负收益，其中 affine
K=20 恶化 0.0695°。

因此 chronological affine 的 0.06–0.07° 收益主要是在纠正当前受约束 checkpoint
残留或引入的用户相关缩放/轴耦合误差，不是“任何强普适模型都需要该适配器”的证据。
研究结论必须同时报告：

1. 普适 checkpoint 自身的未适配精度；
2. 在同一 checkpoint 上的配对个性化增益；
3. 最终 personalized absolute error，而不能只比较 delta。

当前 5 折任务评估的是受约束训练路径；下一阶段需要加入 no-constraint universal folds，
否则无法判断应优先改善基础模型，还是部署个性化 adapter。
