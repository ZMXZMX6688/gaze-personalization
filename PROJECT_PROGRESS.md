...
| 56 用户五折训练 | 已完成 | 5 个 fold 均完成训练 |
| 五折个性化评测 | 已完成 | 56 位用户均被分配到测试 fold，55 位有可行个性化评测 |
| 五折结果汇总 | 已完成 | aggregate 包含 13,020 条结果 |
| 五折完整性审计 | 已完成 | 56/56 恰好分配一次，5 fold 均通过，0 个完整性错误 |
...
| pitch 运输归因 | 已完成 | 同号时 +0.0553°、异号时 -0.0341°；确认失败源于校准—评估偏差运输 |
| 外部确认协议 | 已冻结、待新数据 | pitch-only 单参数、隔离规则、bootstrap 准入和禁止事后调参项已机器固化 |
| 确认实验启动锁 | 已完成 | 固化 56 用户与 10 checkpoint 选择集；自动阻断用户重叠和非新场景 |
...
|---|---|---|---|---|---|
| P0-01 | P0 | 已完成 | 完成约束版 56 用户五折训练与个性化评测 | 5 个 summary 完整，aggregate 成功生成 | 自动任务链 |
| P0-02 | P0 | 已完成 | 审计五折数据完整性 | 56 个 SID 恰好分配一次；校准与评测来源可追踪；输出审计表 | task/P0-02-audit |
