...
研究主线、适配器参数含义、正交性、偏差定义和机制诊断见 [PERSONALIZATION_LOGIC.md](PERSONALIZATION_LOGIC.md)。任何“个性化有效”的结论必须先通过偏差存在性、跨片段稳定性和补偿可辨识性检验，再讨论带门控的最终效果。最新归因表明，两参数 bias 的可迁移信号主要来自 pitch；补偿成败取决于校准期 pitch 偏差能否运输到隔离评估期，而非简单取决于适配器容量。
...
## 研究状态：已经摆脱盲目试错，但尚未完成外部确认

本项目当前不再以“哪个适配器误差最低”为研究主线，而是依次回答五个可证伪问题：

1. 通用模型对同一用户是否存在系统性残差？
2. 该残差在隔离片段之间是否稳定，而不是场景或抽样偶然性？
3. 适配器的每个参数是否可辨识、是否具有明确几何含义？
4. 校准期识别的偏差能否运输到评估期？
5. 补偿收益是否在逐用户宏平均和置信下界上成立？
...
通用模型残差
  → 识别 yaw/pitch 固定偏差
  → 分量消融发现可迁移信号主要来自 pitch
  → 冻结为严格一参数 pitch-only 候选
  → 发现成败由校准—评估 pitch 偏差运输决定
  → 等待全新用户 / checkpoint / 设备的确认性验证
```
...
| 原问题 | 当前处理 |
|---|---|
| 实验零散、缺少主线 | 统一为“偏差存在—稳定—可辨识—运输—补偿—准入”逻辑链 |
| 适配器参数含糊 | 明确 bias=2、pitch-only=1、SO(3)=3、affine=6 个自由参数 |
| 正交性没有说明 | SO(3) 严格满足 \(R^TR=I,\det R=1\)；bias 不涉及矩阵正交；affine 明确允许非正交尺度与剪切 |
| 直接从适配器跳到效果 | 增加 yaw-only、pitch-only、full 分量消融和逐用户运输归因 |
| 只报告最好均值 | 使用 subject macro mean、逐用户配对和单侧 95% bootstrap 下界 |
| 失败无法归因 | 已区分参数噪声、方向覆盖不足、偏差漂移、符号反转和 checkpoint×protocol 交互 |
| 边跑边改、结论污染 | 增加机器可校验的冻结确认协议 `confirmation_protocol.json` |

### 尚未解决的问题

- pitch-only 在 36 个配置中只有 16 个配置的增益置信下界为正，不能称为普适方案；
- 当前校准内部特征无法预测跨 checkpoint 成败，leave-one-suite-out 符号准确率仅 35.71%；
- 评估期 oracle pitch 残差只能用于离线归因，不能作为线上 gate；
- 仍缺一批完全未参与方法筛选的新用户、新 checkpoint 或新设备数据；
- 在外部确认完成前，项目状态是“机制候选已冻结”，不是“部署方案已验证”。

## 适配器参数与证据等级

| 方案 | 参数量 | 参数含义 | 几何约束 | 当前证据等级 |
|---|---:|---|---|---|
| yaw/pitch bias | 2 | 水平、垂直固定角偏差 | 角坐标平移，不涉及正交矩阵 | 已完成大规模基准；yaw 缺乏迁移证据 |
| pitch-only bias | 1 | 垂直固定角偏差 | yaw 严格不变 | 已完成探索性复验；等待外部确认 |
| SO(3) rotation | 3 | 三维轴角旋转 | 严格正交、行列式为 1 | 几何合法，但有限覆盖下不可辨识 |
| tangent affine | 6 | 2×2 线性项和二维偏置 | 非正交，允许尺度、剪切和耦合 | 表达力高，但无门控时易过拟合 |
...
| pitch-only bias | 1 | 只修正垂直方向固定偏差，yaw 严格保持不变 |
...
一句话总结：**历史最优均值不再作为核心结论；当前核心结论是可迁移个性化信号主要来自 pitch，但其有效性仍受 checkpoint 和 protocol 限制。**
...
- **历史结果：** 56 位用户均作为测试用户参与一次，五折汇总覆盖全部用户；K=50 的 chronological affine 获得当时最大宏平均改善，但这只能说明受控基准表现。
- **样本效率：** K=20 的 chronological affine 在 10 次重复中平均改善 0.0465°，Gain 的 5% / 95% 分位数为 +0.0381° / +0.0558°。
- **机制结论：** 两参数 bias 的分量消融显示 yaw-only 仅 3/36 配置平均为正，而 pitch-only 为 23/36。
- **候选结论：** 独立重拟合的一参数 pitch-only 在 22/36 配置平均为正、16/36 置信下界为正；相对两参数 bias 在 28/36 配置改善。
- **运输边界：** 504 个逐用户配置中，校准与评估 pitch 同号时平均 Gain 为 +0.0553°，异号时为 -0.0341°。
